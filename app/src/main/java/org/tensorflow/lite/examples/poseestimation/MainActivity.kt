...
class MainActivity : AppCompatActivity() {
    ...
    private fun createPoseEstimator() {
        ...
        val poseDetector = when (modelPos) {
            ...
            1 -> {
                ...
                showDetectionScore(true)
                showTracker(false)
                MoveNet.create(this, device, ModelType.Thunders)
            }
            2 -> {
    ...
    fun reset(v: View){
        ...
        counter = 0
        start_rep = 0L
        reps_time = mutableListOf<Float>()
        reps_mean_wrist_pos = mutableListOf<Float>()
        reps_median_wrist_pos = mutableListOf<Float>()
        rep_wrist_poses_per_time = mutableListOf<Float>()
    }
    fun save(v: View){
...
//        db.deleteData()
        var i = 0
        var isLast = 0
        for (i_rep in reps_time) {
            if(i + 1 == reps_time.size){
                isLast = 1
            }
            var rep = Rep(reps_time[i], reps_mean_wrist_pos[i], reps_median_wrist_pos[i], isLast)
            var db = DatabaseHandler(this)
            db.insertData(rep)
            i++
        }
